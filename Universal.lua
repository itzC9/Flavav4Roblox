...
local FlavaAssetTable = {["Flava/assets/FlavaCape.png"] = "rbxassetid://13380453812", ["Flava/assets/ArrowIndicator.png"] = "rbxassetid://13350766521"}
...
local entityLibrary = loadstring(FlavaGithubRequest("Libraries/entityHandler.lua"))()
shared.Flavaentity = entityLibrary
do
	entityLibrary.selfDestruct()
	table.insert(FlavaConnections, GuiLibrary.ObjectsThatCanBeSaved.FriendsListTextCircleList.Api.FriendRefresh.Event:Connect(function()
		entityLibrary.fullEntityRefresh()
	end))
	table.insert(FlavaConnections, GuiLibrary.ObjectsThatCanBeSaved["Teams by colorToggle"].Api.Refresh.Event:Connect(function()
		entityLibrary.fullEntityRefresh()
	end))
...
local sha = loadstring(FlavaGithubRequest("Libraries/sha.lua"))()
run(function()
	local olduninject
	...
	function whitelist:playeradded(v, joined)
		if self:get(v) ~= 0 then
			...
			if self.localprio == 0 then
				...
				GuiLibrary.SelfDestruct = function() warningNotification('Flava', 'No escaping the private members :)', 10) end
	...
	function whitelist:checkmessage(msg, plr)
		...
		if self.localprio > 0 and self.said[plr.Name] == nil and msg == 'helloimusinginhaler' and plr ~= lplr then
			...
			notif('Flava', plr.Name..' is using Flava!', 60)
			self.customtags[plr.Name] = {{text = 'Flava USER', color = Color3.new(1, 1, 0)}}
	...
	function whitelist:oldchat(func)
		...
		table.insert(FlavaConnections, {Disconnect = function() hookfunction(func, oldchat) end})
	end
	...
	function whitelist:hook()
		...
		if textChatService.ChatVersion == Enum.ChatVersion.TextChatService then
			if exp then
				if exp:WaitForChild('appLayout', 5) then
					table.insert(FlavaConnections, exp:FindFirstChild('RCTScrollContentView', true).ChildAdded:Connect(function(obj)
		...
		if exp then
			...
			if bubblechat then
				table.insert(FlavaConnections, bubblechat.DescendantAdded:Connect(function(newbubble)
	...
	function whitelist:check(first)
		...
		if not first or whitelist.textdata ~= whitelist.olddata then
			if not first then
				whitelist.olddata = isfile('Flava/profiles/whitelist.json') and readfile('Flava/profiles/whitelist.json') or nil
			end
			...
			if whitelist.textdata ~= whitelist.olddata then
				if whitelist.data.Announcement.expiretime > os.time() then
					...
					if table.find(targets, tostring(lplr.UserId)) then
						...
						hint.Text = 'Flava ANNOUNCEMENT: '..whitelist.data.Announcement.text
						hint.Parent = workspace
						game:GetService('Debris'):AddItem(hint, 20)
					end
				end
				whitelist.olddata = whitelist.textdata
				pcall(function() writefile('Flava/profiles/whitelist.json', whitelist.textdata) end)
			end

			if whitelist.data.KillFlava then
	...
	task.spawn(function()
		repeat
			...
			task.wait(10)
		until shared.FlavaInjected == nil
	end)
	table.insert(FlavaConnections, {Disconnect = function()
...
shared.Flavawhitelist = whitelist
...
run(function()
	...
	local Radar = GuiLibrary.CreateCustomWindow({
		Name = "Radar",
		Icon = "Flava/assets/RadarIcon1.png",
		IconSize = 16
	})
	...
	local radartable = {}
	table.insert(FlavaConnections, Radar.GetCustomChildren().Parent:GetPropertyChangedSignal("Size"):Connect(function()
		RadarFrame.Position = UDim2.new(0, 0, 0, (Radar.GetCustomChildren().Parent.Size.Y.Offset == 0 and 45 or 0))
	end))
	GuiLibrary.ObjectsThatCanBeSaved.GUIWindow.Api.CreateCustomToggle({
		Name = "Radar",
		Icon = "Flava/assets/RadarIcon2.png",
	...
	SilentAim = GuiLibrary.ObjectsThatCanBeSaved.CombatWindow.Api.CreateOptionsButton({
		...
		Function = function(callback)
			if callback then
				...
				task.spawn(function()
					repeat
						FlavaTargetInfo.Targets.SilentAim = SlientAimShotTick >= tick() and SilentAimShot or nil
						task.wait()
				...
				FlavaTargetInfo.Targets.SilentAim = nil
			end
		end,
		ExtraText = function()
							...
							if #plrs > 0 then
								...
								if tool and touch then
									for i,v in pairs(plrs) do
										...
										FlavaTargetInfo.Targets.Killaura = v
										KillauraNearTarget = true
...
run(function()
    ...
    local arrowAddFunction = function(plr)
        ...
        arrowObject.Image = downloadFlavaAsset("Flava/assets/ArrowIndicator.png")
		...
		Function = function(callback)
			if callback then
				...
				table.insert(Cape.Connections, lplr.CharacterAdded:Connect(function(char)
					task.spawn(function()
						pcall(function()
							capeFunction(char, (successfulcustom or downloadFlavaAsset("Flava/assets/FlavaCape.png")))
						end)
					end)
				end))
				if lplr.Character then
					task.spawn(function()
						pcall(function()
							capeFunction(lplr.Character, (successfulcustom or downloadFlavaAsset("Flava/assets/FlavaCape.png")))
						end)
					end)
				end
			else
	...
	GamingChair = GuiLibrary.ObjectsThatCanBeSaved.RenderWindow.Api.CreateOptionsButton({
		...
		Function = function(callback)
			if callback then
				...
				movingsound.SoundId = downloadFlavaAsset("Flava/assets/ChairRolling.mp3")
				movingsound.Volume = 0.4
				movingsound.Looped = true
				movingsound.Parent = workspace
				flyingsound = Instance.new("Sound")
				flyingsound.SoundId = downloadFlavaAsset("Flava/assets/ChairFlying.mp3")
