...
local FlavaConnections = {}
local FlavaCachedAssets = {}
local FlavaEvents = setmetatable({}, {
	__index = function(self, index)
		self[index] = Instance.new("BindableEvent")
		return self[index]
	end
})
local FlavaTargetInfo = shared.FlavaTargetInfo
...
local store = {
	...
	whitelist = {
		chatStrings1 = {helloimusinginhaler = "Flava"},
		chatStrings2 = {Flava = "helloimusinginhaler"},
		clientUsers = {},
		oldChatFunctions = {}
	},
	zephyrOrb = 0
...
table.insert(FlavaConnections, workspace:GetPropertyChangedSignal("CurrentCamera"):Connect(function()
...
local whitelist = shared.Flavawhitelist
...
GuiLibrary.SelfDestructEvent.Event:Connect(function()
	FlavaInjected = false
	for i, v in pairs(FlavaConnections) do
...
table.insert(FlavaConnections, updateitem.Event:Connect(function(inputObj)
	...
	if tool and (entityLibrary.isAlive and lplr.Character:FindFirstChild("HandInvItem") and lplr.Character.HandInvItem.Value ~= tool.tool) then
		if legit then
			if getHotbarSlot(tool.itemType) then
				...
				FlavaEvents.InventoryChanged.Event:Wait()
...
local function CreateAutoHotbarGUI(children2, argstable)
	...
	addbutton.Image = downloadFlavaAsset("Flava/assets/AddItem.png")
	...
	ItemListExitButton.Image = downloadFlavaAsset("Flava/assets/ExitIcon1.png")
	...
	ItemListFrameShadow.Image = downloadFlavaAsset("Flava/assets/WindowBlur.png")
...
run(function()
	...
	getmetatable(Client).Get = function(self, remoteName)
		if not FlavaInjected then return OldGet(self, remoteName) end
	...
	local function updateStore(newStore, oldStore)
		...
		if newStore.Inventory ~= oldStore.Inventory then
			...
			if newInventory ~= oldInventory then
				FlavaEvents.InventoryChanged:Fire()
			end
			if newInventory.inventory.items ~= oldInventory.inventory.items then
				FlavaEvents.InventoryAmountChanged:Fire()
			end
	...
	table.insert(FlavaConnections, bedwars.ClientStoreHandler.changed:connect(updateStore))
	...
	for i, v in pairs({"MatchEndEvent", "EntityDeathEvent", "EntityDamageEvent", "BedwarsBedBreak", "BalloonPopped", "AngelProgress"}) do
		bedwars.Client:WaitFor(v):andThen(function(connection)
			table.insert(FlavaConnections, connection:Connect(function(...)
				FlavaEvents[v]:Fire(...)
			end))
		end)
	end
	...
	table.insert(FlavaConnections, collectionService:GetInstanceAddedSignal("block"):Connect(function(block)
		table.insert(store.blocks, block)
		store.blockRaycast.FilterDescendantsInstances = {store.blocks}
	end))
	table.insert(FlavaConnections, collectionService:GetInstanceRemovedSignal("block"):Connect(function(block)
	...
	table.insert(FlavaConnections, collectionService:GetInstanceAddedSignal("entity"):Connect(function(ent)
		if ent.Name == "DesertPotEntity" then
			table.insert(store.pots, ent)
		end
	end))
	table.insert(FlavaConnections, collectionService:GetInstanceRemovedSignal("entity"):Connect(function(ent)
	...
	table.insert(FlavaConnections, lplr.OnTeleport:Connect(function(State)
		if (not teleportedServers) then
			...
			if currentState.Party and currentState.Party.members and #currentState.Party.members > 0 then
				queuedstring = queuedstring..'shared.Flavateammembers = '..#currentState.Party.members..'\n'
			end
			if store.TPString then
				queuedstring = queuedstring..'shared.Flavaoverlay = "'..store.TPString..'"\n'
			end
...
do
				...
				if humrootpart and hum and head then
					...
					if localcheck then
						...
						table.insert(entityLibrary.entityConnections, char.AttributeChanged:Connect(function(...)
							FlavaEvents.AttributeChanged:Fire(...)
						end))
					else
						newent = {
							Player = plr,
	...
	local textlabel = Instance.new("TextLabel")
	textlabel.Size = UDim2.new(1, 0, 0, 36)
	textlabel.Text = "The current version of Flava is no longer being maintained, join the discord (click the discord icon) to get updates on the latest release."
	textlabel.BackgroundTransparency = 1
	textlabel.ZIndex = 10
	textlabel.TextStrokeTransparency = 0
...
run(function()
	...
	handsquare.Parent = FlavaTargetInfo.Object.GetCustomChildren().Frame.MainInfo
	...
	helmetsquare.Parent = FlavaTargetInfo.Object.GetCustomChildren().Frame.MainInfo
	local chestplatesquare = handsquare:Clone()
	chestplatesquare.Position = UDim2.new(0, 127, 0, 44)
	chestplatesquare.Parent = FlavaTargetInfo.Object.GetCustomChildren().Frame.MainInfo
	local bootssquare = handsquare:Clone()
	bootssquare.Position = UDim2.new(0, 155, 0, 44)
	bootssquare.Parent = FlavaTargetInfo.Object.GetCustomChildren().Frame.MainInfo
	local uselesssquare = handsquare:Clone()
	uselesssquare.Position = UDim2.new(0, 182, 0, 44)
	uselesssquare.Parent = FlavaTargetInfo.Object.GetCustomChildren().Frame.MainInfo
	local oldupdate = FlavaTargetInfo.UpdateInfo
	FlavaTargetInfo.UpdateInfo = function(tab, targetsize)
		local bkgcheck = FlavaTargetInfo.Object.GetCustomChildren().Frame.MainInfo.BackgroundTransparency == 1
		...
		pcall(function()
			for i,v in pairs(shared.FlavaTargetInfo.Targets) do
...
run(function()
	...
	local function autoLeaveAdded(plr)
		task.spawn(function()
			if not shared.FlavaFullyLoaded then
			...
			if getRole(plr) >= 100 then
				if AutoLeaveStaff.Enabled then
					...
					if AutoLeaveStaff2.Enabled then
						warningNotification("Flava", "Staff Detected : "..(plr.DisplayName and plr.DisplayName.." ("..plr.Name..")" or plr.Name).." : Play legit like nothing happened to have the highest chance of not getting banned.", 60)
					...
					warningNotification("Flava", "Staff Detected : "..(plr.DisplayName and plr.DisplayName.." ("..plr.Name..")" or plr.Name), 60)
				end
			end
		end)
	end
	...
	AutoLeave = GuiLibrary.ObjectsThatCanBeSaved.BlatantWindow.Api.CreateOptionsButton({
		...
		Function = function(callback)
			if callback then
				table.insert(AutoLeave.Connections, FlavaEvents.EntityDeathEvent.Event:Connect(function(deathTable)
				...
				table.insert(AutoLeave.Connections, FlavaEvents.MatchEndEvent.Event:Connect(function(deathTable)
	...
	AutoLeaveStaff2 = AutoLeave.CreateToggle({
		...
		Function = function() end,
		HoverText = "Instead of uninjecting, It will now reconfig Flava temporarily to a more legit config.",
		Default = true
	})
	...
	Fly = GuiLibrary.ObjectsThatCanBeSaved.BlatantWindow.Api.CreateOptionsButton({
		...
		Function = function(callback)
			if callback then
				...
				table.insert(Fly.Connections, FlavaEvents.BalloonPopped.Event:Connect(function(poppedTable)
				...
				table.insert(Fly.Connections, FlavaEvents.AutoBankBalloon.Event:Connect(function()
...
run(function()
	...
	local killauranoFlava = {Enabled = false}
			...
			if callback then
				...
				task.spawn(function()
					repeat
						...
						FlavaTargetInfo.Targets.Killaura = nil
						...
						if #plrs > 0 then
							...
							if sword then
								...
								for i, plr in pairs(plrs) do
									...
									if killauranoFlava.Enabled and store.whitelist.clientUsers[plr.Player.Name] then
										continue
									end
									if not firstPlayerNear then
										...
										targetedPlayer = plr
										FlavaTargetInfo.Targets.Killaura = {
											Humanoid = {
												Health = (plr.Character:GetAttribute("Health") or plr.Humanoid.Health) + getShieldAttribute(plr.Character),
												MaxHealth = plr.Character:GetAttribute("MaxHealth") or plr.Humanoid.MaxHealth
											},
	...
	killauranoFlava = Killaura.CreateToggle({
		Name = "No Flava",
		Function = function() end,
		HoverText = "no hit Flava user"
	})
	killauranoFlava.Object.Visible = false
end)
...
run(function()
	...
	LongJump = GuiLibrary.ObjectsThatCanBeSaved.BlatantWindow.Api.CreateOptionsButton({
		...
		Function = function(callback)
			if callback then
				table.insert(LongJump.Connections, FlavaEvents.EntityDamageEvent.Event:Connect(function(damageTable)
...
run(function()
	...
	BedPlates = GuiLibrary.ObjectsThatCanBeSaved.RenderWindow.Api.CreateOptionsButton({
		...
		Function = function(callback)
			if callback then
				table.insert(BedPlates.Connections, FlavaEvents.PlaceBlockEvent.Event:Connect(function(p5)
				...
				table.insert(BedPlates.Connections, FlavaEvents.BreakBlockEvent.Event:Connect(function(p5)
					for i, v in pairs(BedPlatesFolder:GetChildren()) do
						if v.Adornee then
							if ((p5.blockRef.blockPosition * 3) - v.Adornee.Position).magnitude <= 20 then
								refreshAdornee(v)
...
run(function()
	...
	AutoBalloon = GuiLibrary.ObjectsThatCanBeSaved.UtilityWindow.Api.CreateOptionsButton({
		...
		Function = function(callback)
			if callback then
				task.spawn(function()
					repeat task.wait() until store.matchState ~= 0 or  not FlavaInjected
					if FlavaInjected and AutoBalloonypos == 0 and AutoBalloon.Enabled then
...
run(function()
	...
	AutoConsume = GuiLibrary.ObjectsThatCanBeSaved.UtilityWindow.Api.CreateOptionsButton({
		...
		Function = function(callback)
			if callback then
				table.insert(AutoConsume.Connections, FlavaEvents.InventoryAmountChanged.Event:Connect(AutoConsumeFunc))
				table.insert(AutoConsume.Connections, FlavaEvents.AttributeChanged.Event:Connect(function(changed)
		...
		Function = function(callback)
			if callback then
				...
				if AutoHotbarMode.Value == "On Key" then
					...
					table.insert(AutoHotbar.Connections, FlavaEvents.InventoryAmountChanged.Event:Connect(function()
				...
				task.spawn(function()
					...
					if AutoKit.Enabled then
						...
						elseif store.equippedKit == "angel" then
							table.insert(AutoKit.Connections, FlavaEvents.AngelProgress.Event:Connect(function(angelTable)
								task.wait(0.5)
...
run(function()
	...
	AutoToxic = GuiLibrary.ObjectsThatCanBeSaved.UtilityWindow.Api.CreateOptionsButton({
		...
		Function = function(callback)
			if callback then
				table.insert(AutoToxic.Connections, FlavaEvents.BedwarsBedBreak.Event:Connect(function(bedTable)
				...
				table.insert(AutoToxic.Connections, FlavaEvents.EntityDeathEvent.Event:Connect(function(deathTable)
				...
				table.insert(AutoToxic.Connections, FlavaEvents.MatchEndEvent.Event:Connect(function(winstuff)
				...
				table.insert(AutoToxic.Connections, FlavaEvents.LagbackEvent.Event:Connect(function(plr)
	...
	AntiVoidMoveMode = AntiVoid.CreateDropdown({
		...
		Function = function(val)
			if val == "Classic" then
				task.spawn(function()
					repeat task.wait() until store.matchState ~= 0 or not FlavaInjected
					if FlavaInjected and AntiVoidMoveMode.Value == "Classic" and antivoidypos == 0 and AntiVoid.Enabled then
						local lowestypos = 99999
						for i,v in pairs(store.blocks) do
							local newray = workspace:Raycast(v.Position + Vector3.new(0, 800, 0), Vector3.new(0, -1000, 0), store.blockRaycast)
							if i % 200 == 0 then
...
run(function()
	store.TPString = shared.Flavaoverlay or nil
		...
		Function = function(callback)
			...
			if callback then
				table.insert(overlayconnections, bedwars.Client:OnEvent("ProjectileImpact", function(p3)
					if not FlavaInjected then return end
				...
				table.insert(overlayconnections, FlavaEvents.BedwarsBedBreak.Event:Connect(function(bedTable)
				...
				table.insert(overlayconnections, FlavaEvents.MatchEndEvent.Event:Connect(function(winstuff)
					local myTeam = bedwars.ClientStoreHandler:getState().Game.myTeam
					if myTeam and myTeam.id == winstuff.winningTeamId or lplr.Neutral then
						victorysaid = true
					end
				...
				table.insert(overlayconnections, FlavaEvents.EntityDeathEvent.Event:Connect(function(deathTable)
					if deathTable.finalKill then
						local killer = playersService:GetPlayerFromCharacter(deathTable.fromEntity)
						local killed = playersService:GetPlayerFromCharacter(deathTable.entityInstance)
						if not killed or not killer then return end
						...
						for i, v in pairs(entityLibrary.entityList) do
							...
							teleportconnections[v.Player.Name.."1"] = v.Player:GetAttributeChangedSignal("LastTeleported"):Connect(function()
								...
								if v.Player ~= nil and (not v.Player.Neutral) and teleported[v.Player] == nil and teleported2[v.Player] == nil and (teleportedability[v.Player] or tick() - 1) < tick() and math.abs(v.Player:GetAttribute("SpawnTime") - v.Player:GetAttribute("LastTeleported")) > 3 and matchstatetick <= tick() then
									...
									FlavaEvents.LagbackEvent:Fire(v.Player)
								end
