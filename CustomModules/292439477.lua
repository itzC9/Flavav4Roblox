...
local function GetURL(scripturl)
	if shared.FlavaDeveloper then
		assert(betterisfile("Flava/"..scripturl), "File not found : Flava/"..scripturl)
		return readfile("Flava/"..scripturl)
	else
		local res = game:HttpGet("https://raw.githubusercontent.com/itzC9/FlavaV4Roblox/main/"..scripturl, true)
		assert(res ~= "404: Not Found", "File not found : Flava/"..scripturl)
		return res
	end
end
...
if shared.FlavaWaiting then repeat task.wait() until shared.RequireTable end
if shared.RequireTable == nil then 
	if shared.RequireTable == nil then
		if queueteleport then
            ...
            prompt:_open("Your exploit is unsupported by Phantom Forces Flava.")
            task.wait(9e9)
		end
	end
end
...
local WhitelistFunctions = shared.Flavawhitelist
...
local teleportfunc = game:GetService("Players").LocalPlayer.OnTeleport:Connect(function(State)
	if (not teleported) and (not shared.FlavaIndependent) then
		teleported = true
		local finaltp = (tpstring and 'shared.Flavaoverlay = "'..tpstring..'"\n' or "").."shared.scriptalreadyjoinedservers = '"..httpservice:JSONEncode(alreadyjoined).."'\n"..bypassScript
...
runcode(function()
	tpstring = shared.Flavaoverlay or nil
