...
local function createwarning(title, text, delay)
	pcall(function()
		local frame = GuiLibrary["CreateNotification"](title, text, delay, "Flava/assets/WarningNotification.png")
...
local function getcustomassetfunc(path)
	if not isfile(path) then
		...
		local req = requestfunc({
			Url = "https://raw.githubusercontent.com/itzc9/FlavaV4ForRoblox/main/"..path:gsub("Flava/assets", "assets"),
			Method = "GET"
		})
