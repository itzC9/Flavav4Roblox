...
local GuiLibrary
local baseDirectory = (shared.VapePrivate and "vapeprivate/" or "flava/")
...
local function downloadVapeAsset(path)
	if customassetcheck then
		if not isfile(path) then
			...
			local suc, req = pcall(function() return vapeGithubRequest(path:gsub("flava/assets", "assets")) end)
			if suc and req then
				writefile(path, req)
			else
				return ""
...
assert(not shared.VapeExecuted, "Flava Already Injected")
...
task.spawn(function()
	...
	if success and assetver > readfile("flava/assetsversion.txt") then
		...
		if isfolder("flava/assets") and not shared.VapeDeveloper then
			if delfolder then
...
task.spawn(function()
	...
	image.Image = downloadVapeAsset("flava/assets/CombatIcon.png")
...
ProfilesTextList = Profiles.CreateTextList({
	...
	CustomFunction = function(profileObject, profileName) 
		...
		bindimg.Image = downloadVapeAsset("flava/assets/KeybindIcon.png")
		...
		bindbkg.MouseEnter:Connect(function() 
			bindimg.Image = downloadVapeAsset("flava/assets/PencilIcon.png") 
			bindimg.Visible = true
			bindtext.Visible = false
			bindbkg.Size = UDim2.new(0, 20, 0, 21)
			bindbkg.Position = UDim2.new(1, -50, 0, 6)
...
OnlineProfilesButtonImage.Image = downloadVapeAsset("flava/assets/OnlineProfilesButton.png")
...
OnlineProfilesExitButton.Image = downloadVapeAsset("flava/assets/ExitIcon1.png")
...
OnlineProfilesFrameShadow.Image = downloadVapeAsset("flava/assets/WindowBlur.png")
...
OnlineProfilesFrameIcon.Image = downloadVapeAsset("flava/assets/ProfilesIcon.png")
...
OnlineProfilesButton.MouseButton1Click:Connect(function()
	...
	if not profilesLoaded then
        ...
        local success, result = pcall(function()
            return game:GetService("HttpService"):JSONDecode(game:HttpGet("https://raw.githubusercontent.com/itzC9/VapeProfiles/main/Profiles/"..saveplaceid.."/profilelist.txt", true))
        end)
		for i,v in pairs(success and result or {}) do 
			onlineprofiles[i] = v
		end
		for i2,v2 in pairs(onlineprofiles) do
			local profileurl = "https://raw.githubusercontent.com/itzC9/VapeProfiles/main/Profiles/"..saveplaceid.."/"..v2.OnlineProfileName
...
FlavaLogo.Image = downloadVapeAsset("flava/assets/FlavaLogo3.png")
...
FlavaLogoV4.Image = downloadVapeAsset("flava/assets/FlavaLogo4.png")
...
VapeText.Parent = FlavaLogoFrame
...
VapeCustomText.Text = ""
VapeCustomText.Parent = FlavaLogoFrame
...
VapeBackground.Parent = FlavaLogoFrame
...
VapeScale.Parent = FlavaLogoFrame
	...
	if scaledgui and scaledgui.Visible then
        ...
        if TextGUI.GetCustomChildren().Parent then
            if (TextGUI.GetCustomChildren().Parent.Position.X.Offset + TextGUI.GetCustomChildren().Parent.Size.X.Offset / 2) >= (gameCamera.ViewportSize.X / 2) then
                ...
                VapeText.Position = UDim2.new(1, -158, 0, (FlavaLogo.Visible and (TextGUIBackgroundToggle.Enabled and 41 or 35) or 5) + 5 + (VapeCustomText.Visible and 25 or 0) - offsets[3])
                VapeCustomText.Position = UDim2.fromOffset(0, FlavaLogo.Visible and 35 or 0)
                ...
                VapeText.Position = UDim2.fromOffset(6, (FlavaLogo.Visible and (TextGUIBackgroundToggle.Enabled and 41 or 35) or 5) + 5 + (VapeCustomText.Visible and 25 or 0) - offsets[3])
				VapeCustomText.Position = UDim2.fromOffset(0, FlavaLogo.Visible and 35 or 0)
				VapeCustomText.TextXAlignment = Enum.TextXAlignment.Left
	...
	Function = function(val)
		...
		if val == "Drawing" then
			...
			table.insert(TextGUIConnections, FlavaLogo:GetPropertyChangedSignal("Visible"):Connect(function()
				FlavaLogoDrawing.Visible = FlavaLogo.Visible
				FlavaLogoV4Drawing.Visible = FlavaLogo.Visible
				FlavaLogoShadowDrawing.Visible = FlavaLogo.Visible and VapeTextExtra.Visible
				FlavaLogo4Drawing.Visible = FlavaLogo.Visible and VapeTextExtra.Visible
			end))
...
TargetInfoHealthBackgroundShadow.Image = downloadVapeAsset("flava/assets/WindowBlur.png")
